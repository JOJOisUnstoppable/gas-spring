...
interface LockableStickyNavProps {
  navigationItems: string[];
  className?: string;
}

export default function LockableStickyNav({ navigationItems, className }: LockableStickyNavProps) {
  const [activeId, setActiveId] = useState<string>('')

  // 映射导航项
  const items = [
    { id: 'hero', label: navigationItems[0] },
    { id: 'why', label: navigationItems[1] },
    { id: 'technology', label: navigationItems[2] },
    { id: 'TypeSelection', label: navigationItems[3] },
    { id: 'forcePerformance', label: navigationItems[4] },
    { id: 'manufacturing', label: navigationItems[5] },
    { id: 'solutions', label: navigationItems[6] },
    { id: 'applications', label: navigationItems[7] },
    { id: 'faq', label: navigationItems[8] },
    { id: 'cta', label: navigationItems[9] }
  ].filter(item => item.label && item.label.trim() !== '');

  const scrollToElement = (elementId: string) => {
    const element = document.getElementById(elementId);
    if (element) {
      // headerOffset = Navbar的高度 (约73px) + StickyNav的高度 (约55px) + 缓冲空间
