...
interface CompressionStickyNavProps {
  navigationItems: string[];
  className?: string;
}

export default function CompressionStickyNav({ navigationItems, className }: CompressionStickyNavProps) {
  const [activeId, setActiveId] = useState<string>('')

  // 映射导航项
  // 注意：这里的 ID 必须与页面中各 Section 的 ID 一致
  const items = [
    { id: 'overview', label: navigationItems[0] },
    { id: 'specs', label: navigationItems[1] },
    { id: 'testing', label: navigationItems[2] },
    { id: 'cta', label: navigationItems[3] } 
